...
namespace CaisseSuperMarcher
{
    class TicketDeCaisse
    {
        ...
        public TicketDeCaisse(List<Produit> P)
        {
        ...
        public void CumuleAchat()
        {
            foreach(Produit p in ListeDesProduits)
            {
                ...
                //ListeDesProduits.Remove(p); //supprimer ce dernier de la liste des produits a acheter
            }
        }
        public ParamClient getParamClient(int idClient)
        {
            ChargementParametre handler = new ChargementParametre();
            return handler.involve(idClient);
